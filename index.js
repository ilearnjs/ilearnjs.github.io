(function() {
  ...
  const linksRegex = re.protocols + re.domainName + re.topLevelDomains + re.port +
    re.path + re.query + re.fragment;

  const hashTagsRegex = '#[a-zA-Z0-9]+';
  ...
  function extractEntities(text) {
    ...
    const regex = new RegExp(`(${hashTagsRegex})|(${usersRegex})|(${linksRegex})`, 'g');
    let fromIndex = 0;
    let start, end;
    (text.match(regex) || []).forEach(s => {
      if (s.startsWith('#')) {
        start = text.indexOf(s, fromIndex);
        end = text.indexOf(s, fromIndex) + s.length - 1;
        fromIndex = end;
  ...
  function handle(e) {
    ...
    outputRawEl.innerText = JSON.stringify(output, null, 2);
    renderCard(output);
  };

  function renderCard({ text, entities }) {
    outputCardEl.innerHTML = text;
    const entitiesList = [
      ...entities.hashTags,
    ...
    entitiesList.forEach(({ fullValue, start, end }) => {
      const el = document.createElement('span');
      el.classList.add('entity');
      el.innerText = fullValue;
      outputCardEl.innerHTML = outputCardEl.innerHTML.substring(0, start) + el.outerHTML + outputCardEl.innerHTML.substring(end + 1);
    });
  };
